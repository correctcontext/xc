...
xc::virsh::get-net-xml(){
  ...
  cat <<EOF
    <network>
      ...
      <ip address='$ip_start' netmask='$ip_mask'>
        <dhcp>
...
xc::virsh::get-node-xml(){
  local network=${1?virsh network name}
  local node_name=${2?virsh node name}
  local image_path=${3?virsh qcow2 image path}
  local memory_bytes=${4:-1024000}  # memory in bytes
  local cpu_count=${5:-1}
  cat <<EOF
    <domain type='kvm'>
      <name>${node_name}</name>
      <memory>${memory_bytes}</memory>
      <currentMemory>${memory_bytes}</currentMemory>
      <vcpu>${cpu_count}</vcpu>
      <os>
...
xc::virsh::ensure-network(){
  if ! xc::virsh::does-network-exist "$1";then
    virsh net-create  <(xc::virsh::get-net-xml "$1" "$2" "$3" "$4")
  fi
}
